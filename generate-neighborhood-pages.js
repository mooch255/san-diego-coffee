...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .footer-subscribe { flex-direction: column; gap: 0.5rem; margin-bottom: 0.85rem; }
        .nl-form { display: flex; flex-direction: column; align-items: center; gap: 0.4rem; width: 100%; max-width: 420px; margin: 0 auto; }
        .nl-label { font-size: 0.85rem; color: rgba(196,167,125,0.9); }
        .nl-input-wrap { display: flex; gap: 0.5rem; width: 100%; }
        .nl-input-wrap input[type=email] { flex: 1; min-width: 0; padding: 0.55rem 0.7rem; border: 1px solid rgba(196,167,125,0.4); border-radius: 4px; background: rgba(255,255,255,0.06); color: #EDE0CC; font-size: 0.95rem; }
        .nl-input-wrap input[type=email]::placeholder { color: rgba(196,167,125,0.5); }
        .nl-input-wrap button { padding: 0.55rem 1rem; background: var(--accent-red, #B85C38); color: #fff; border: none; border-radius: 4px; font-weight: 600; cursor: pointer; white-space: nowrap; }
        .nl-input-wrap button:disabled { opacity: 0.6; cursor: default; }
        .nl-hp { position: absolute; left: -9999px; width: 1px; height: 1px; opacity: 0; }
        .nl-msg { font-size: 0.85rem; min-height: 1em; }
        .nl-msg.ok { color: #9BD0A0; }
        .nl-msg.err { color: #E4A08A; }
        .nl-fineprint { font-size: 0.72rem; opacity: 0.6; }
        .nl-fineprint a { color: inherit; }
    ...
    <footer>
        <div class="footer-row footer-subscribe">
          <form class="nl-form" novalidate>
            <label for="nlEmail" class="nl-label">Get occasional SD coffee news &amp; events:</label>
            <div class="nl-input-wrap">
              <input type="email" id="nlEmail" name="email" placeholder="you@email.com" autocomplete="email" required aria-label="Email address">
              <input type="text" name="website_hp" tabindex="-1" autocomplete="off" aria-hidden="true" class="nl-hp">
              <button type="submit">Subscribe</button>
            </div>
            <span class="nl-msg" role="status" aria-live="polite"></span>
          </form>
        </div>
        <script>
        document.addEventListener('DOMContentLoaded', function () {
          ...
          form.addEventListener('submit', async function (e) {
            ...
            if (!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(email)) {
              msg.textContent = 'Please enter a valid email.'; msg.className = 'nl-msg err'; return;
            }
            btn.disabled = true; btn.textContent = '…'; msg.textContent = '';
            try {
              var res = await fetch('/.netlify/functions/submit-to-sheets', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ submissionType: 'newsletter', email: email, website_hp: form.querySelector('input[name=website_hp]').value, source: window.location.pathname })
              });
              if (!res.ok) throw new Error('bad response');
              msg.textContent = "Thanks — you're on the list! ☕"; msg.className = 'nl-msg ok';
              form.reset();
            } catch (err) {
              msg.textContent = 'Something went wrong. Try again.'; msg.className = 'nl-msg err';
            } finally {
