...
- Neighborhood search with smart geographic detection (50+ San Diego areas)
- Location detail pages with photo galleries
- Online-only roaster support (no physical location required)
- Curated neighborhood + specialty **Guides** (`/guides`) with editorial picks and FAQs
- **@sdcoffeeguide drink ratings** (cortado + signature drink scores) displayed on individual location pages
- Per-neighborhood landing pages at `/neighborhoods/{slug}/` for SEO
...
## Guides
Curated editorial collections rendered by `guide.html` (single-page template). Two types: `neighborhood` and `specialty`.
- `guides.js` — global `window.GUIDES` array; each entry has `id`, `type`, `title`, `displayTitle`, `subtitle`, `heroImage`, `excerpt`, `locationCount`, SEO fields, `about{}`, `quickStats{}`, `faq[]`, and `locations[]` (with `loc_XXX` ids + per-pick blurbs)
- `guides.html` — hub/listing page
- `guide.html` — template rendered for each guide via `?slug=` or `/guides/{slug}/` path
- Hero image at `/images/guides/{slug}-hero.jpg`; OG image at `/images/guides/{slug}-og.jpg` (optional, falls back to hero)
- `guide.html` keeps an inline `TITLES` map keyed by slug for Stage-1 SEO indexing — update it when adding a new guide (see SEO Critical Rules below)
- FAQs render as accordion + `FAQPage` JSON-LD for rich-result eligibility

**To add a new guide:** append to `guides.js`, drop hero image, update `TITLES` map in `guide.html`, add to `sitemap.xml`.

## SD Coffee Guide Ratings (`sdcoffeeguide.js`)
Hand-maintained drink ratings + review excerpts from the @sdcoffeeguide Instagram account, displayed inline on individual location pages.
- `window.SDCG_RATINGS` — object keyed by `loc_XXX` id
...
### Edge function: server-side OG injection + legacy URL redirects
`netlify/edge-functions/og-meta.js` intercepts `/highlight.html`, `/blog-post.html`, `/location.html`, and `/locations/*` and does two things:

1. **301 redirects** legacy `loc_XXX` URLs (both `?id=loc_XXX` and `/locations/loc_XXX`) → canonical `/locations/{slug}`. Fixes "soft 404" reports in Google Search Console from duplicate URLs.
2. **Rewrites OG/Twitter/title/description meta tags** in the HTML response before it reaches the crawler. Reads `highlights.js` / `blog.js` / `locations.js` from the same origin at runtime (cached per edge instance) — no manual data duplication when adding new entries.

The slug-computation logic must stay in sync with `locationSlug()` in `location.html`. If you change slug rules in one, update the other.

Fails open: if data fetch fails, request passes through with default tags — never blocks the response. To add OG injection for a new template page type, add its path to `export const config = { path: [...] }` at the bottom of the file.

### Homepage OG image
- `/og-image.png` (1200×630, ~22KB) — stylized SD map with cup-pin markers + "San Diego's Coffee Directory · 200+ specialty cafes & roasters · 50+ neighborhoods" headline card
- Source: `og-image.svg` — keep as editable source of truth
- Re-export to PNG with `sharp` (already in devDependencies):
  ```
  node -e "require('sharp')('og-image.svg').resize(1200,630).png({quality:90}).toFile('og-image.png').then(i=>console.log(i))"
  ```
- All static pages reference `https://sandiegocoffee.co/og-image.png`. Per-page overrides for highlight/blog-post/location pages are handled by the edge function above.
...
### Sitemap
- `sync-locations.js` automatically updates location entries in `sitemap.xml` (regen handled by `generate-sitemap.js`, invoked from `sync-locations.js`)
- Location URLs in the sitemap use canonical slug form: `/locations/{slug}` (not legacy `?id=loc_XXX`)
...
Static `/neighborhoods/{slug}/index.html` pages exist for major neighborhoods (la-jolla, north-park, oceanside, etc.) — each is its own HTML file with unique title/description for SEO. Generated by `generate-neighborhood-pages.js` from `neighborhoods.js`. Re-run that script if you add a new neighborhood to the list or change the template.
